...
impl BedFile {
    ...
    /// Stream decoded column-chunks from the .bed file.
    /// Returns an iterator of (start_snp_index, decoded_chunk) pairs.
    /// Each chunk is n_samples × chunk_cols (last chunk may be smaller).
    pub fn stream_chunks(
        &self,
        chunk_size: usize,
        subset: &SubsetSpec,
    ) -> Box<dyn Iterator<Item = (usize, Array2<f64>)> + '_> {
        match subset {
            SubsetSpec::All => {
                let bps = self.bytes_per_snp();
                let n = self.n_samples;
                let total = self.n_snps;
                let data = &self.mmap[3..]; // skip magic bytes

                Box::new((0..total).step_by(chunk_size).map(move |start| {
                    let end = (start + chunk_size).min(total);
                    let this_chunk = end - start;
                    let packed = &data[start * bps..end * bps];
                    let decoded = decode_bed_chunk(packed, n, this_chunk);
                    (start, decoded)
                }))
            }
            SubsetSpec::Rate(rate) => {
                // Deterministic thinning: take every 1/rate-th SNP
                let step = (1.0 / rate).ceil() as usize;
                let indices: Vec<usize> = (0..self.n_snps).step_by(step).collect();
                self.stream_chunks_by_indices(chunk_size, indices)
            }
            SubsetSpec::Indices(indices) => {
                self.stream_chunks_by_indices(chunk_size, indices.clone())
            }
        }
    }

    fn stream_chunks_by_indices(
        &self,
        chunk_size: usize,
        indices: Vec<usize>,
    ) -> Box<dyn Iterator<Item = (usize, Array2<f64>)> + '_> {
        ...
        Box::new(
            indices
                ...
                .map(move |chunk_indices| {
                    ...
                    let this_chunk = chunk_indices.len();
                    let mut out = Array2::<f64>::zeros((n, this_chunk));

                    for (col, &snp_idx) in chunk_indices.iter().enumerate() {
                        let snp_bytes = &data[snp_idx * bps..(snp_idx + 1) * bps];
                        decode_single_snp(snp_bytes, n, out.column_mut(col));
                    }
                    (first_idx, out)
                }),
        )
